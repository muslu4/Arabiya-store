...
class ProductAdmin(admin.ModelAdmin):
    ...
    def rating_display(self, obj):
        ...
        if obj.average_rating > 0:
            ...
            return format_html(
                '<div>{} <span class="badge badge-primary">{:.1f}</span></div>',
                stars, obj.average_rating
            )
        return format_html('<span class="badge badge-light">لا توجد تقييمات</span>')
    ...
    def actions_column(self, obj):
        ...
        # View reviews button
        reviews_url = reverse('admin:products_product_changelist') + f'?reviews__product__id__exact={obj.id}'
    ...
    def main_image_preview(self, obj):
        ...
        return format_html('<div style="width: 200px; height: 150px; background: #f8f9fa; border-radius: 8px; display: flex; align-items: center; justify-content: center;"><i class="fas fa-image" style="color: #6c757d; font-size: 2em;"></i></div>')
    ...
    def reviews_count(self, obj):
        ...
        return format_html(
            '<span class="badge badge-info">{} تقييم</span>',
            count
        )
    ...
    make_featured.short_description = 'تخصيص المنتجات المحددة'

    def remove_featured(self, request, queryset):
        """Remove featured status from selected products"""
        queryset.update(is_featured=False)
    remove_featured.short_description = 'إزالة التخصيص من المنتجات المحددة'

    def apply_discount(self, request, queryset):
        """Apply discount to selected products"""
        # This would typically open a form to specify discount percentage
        # For simplicity, we'll apply a 10% discount
